...
# handsoff multiagent with middlware
...
def escalate_to_human(reason: str) -> str:
    ...
    return f"Escalating to human support. Reason: {reason}" #automaticaly converted to ToolMessage by the calling agent
...
def go_back_to_warranty(runtime : ToolRuntime[None, SupportState]) -> Command:
    ...
    return Command(
        update={
            "messages": [
                            ToolMessage(
                                content=f"Going back to warranty verification step.",
                                tool_call_id=runtime.tool_call_id,
                            )
                        ],
...
def go_back_to_classification(runtime : ToolRuntime[None, SupportState]) -> Command:
    ...
    return Command(
        update={
            "messages": [
                ToolMessage(
                    content=f"Going back to issue classification step.",
                    tool_call_id=runtime.tool_call_id,
                )
            ],
            "current_step": "issue_classifier"
...
if __name__ == "__main__":
    # Testing the multi-step agent with a simulated conversation
