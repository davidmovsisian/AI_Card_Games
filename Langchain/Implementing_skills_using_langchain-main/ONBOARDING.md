...
15. [Bugs Fixed — Change Log](#15-bugs-fixed--change-log)
...
## 15. Bugs Fixed — Change Log

This section documents every bug discovered and fixed post-initial-build, so new joiners understand what was broken and why.

---

### Bug 1 — Transcript response blank (skills + tokens showing, no content)

**Symptom:** The chat response shows the skill badge (`🔧 youtube-transcript`) and token count, but the message body is completely empty.

**Root cause:** Two issues combined:

1. `app.py` was re-processing `result["response"]` with a list-comprehension after `run_agent()` had already returned a clean string:
```python
# BROKEN — raw is already a str, this list comprehension produces ""
response = (
    "\n".join(b.get("text") or "" for b in raw if isinstance(b, dict)).strip()
    if isinstance(raw, list) else str(raw).strip()
)
```
2. Even when the extraction was correct, Gemini sometimes returned an **empty final synthesis** — it ran the tools correctly but produced no text response (likely due to output budget pressure from large transcript content).

**Fixes applied:**

`app.py` — use `result["response"]` directly (already cleaned by `extract_text_content()`):
```python
response = result["response"]   # always a clean str — no re-processing needed
```

`app.py` — added a **smart fallback renderer**: if `response` is empty but tools ran, extract the best field from each tool's raw JSON result (`transcript` → `formatted_with_timestamps` → `summary` → `result` → `text`) and display it with a note:
```
⚠️ The model returned an empty synthesis — showing raw tool output below.
```

`skill_agent.py` — added `"tool_results"` and `"result_full"` to the `run_agent()` return dict so `app.py` has access to the complete tool outputs (not just the 500-char preview).

---

### Bug 2 — `youtube-transcript-api` v1.x breaking change

**Symptom:**
```
[Tool] ← {"error": "'FetchedTranscriptSnippet' object is not subscriptable", "error_type": "TypeError"}
```

**Root cause:** `youtube-transcript-api` v1.x changed `transcript.fetch()` to return `FetchedTranscriptSnippet` **objects** instead of plain dicts. The old script used dict-style access everywhere:
```python
item['text']      # ← TypeError in v1.x
item['start']     # ← TypeError in v1.x
item['duration']  # ← TypeError in v1.x
```

**Fix applied** — `extract_transcript.py` — added a `_segment_to_dict()` normalisation function that detects the type at runtime and returns a plain dict regardless of API version:
```python
def _segment_to_dict(segment) -> Dict:
    if isinstance(segment, dict):               # v0.x
        return {"text": segment.get("text", ""), ...}
    return {                                     # v1.x object
...
```
All three segment access points in `get_transcript()` and `get_transcript_with_timestamps()` now call this function. No restart needed — the script is `importlib.reload()`-ed on every tool call.

---

### Bug 3 — Agent double-looping (tools called twice)

**Symptom:** Console shows the same tool sequence executing twice:
```
[Tool] → read_skill_instructions(...)
[Tool] → extract_youtube_transcript(...)
[Tool] → read_skill_instructions(...)   ← duplicate!
[Tool] → extract_youtube_transcript(...)  ← duplicate!
```

**Root cause:** After the first tool pass returns results via `ToolMessage`, Gemini re-enters the agent node and — seeing the conversation history but not fully trusting it — decides to re-execute the workflow from scratch. With `recursion_limit=25`, it could loop up to 12 full cycles before hitting the wall.

**Three-part fix applied:**

**1. Dynamic "already done" block in the system prompt** (`skill_agent.py` — `build_system_prompt()`):

After each tool pass, `_agent_node()` passes the current `tool_results` list into `build_system_prompt()`. This builds a live section injected into the system prompt:

```
## ✅ Tools Already Executed This Turn (DO NOT call again)
  - read_skill_instructions({...}) → DONE
  - extract_youtube_transcript({...}) → DONE

All results are already in the conversation above.
Write your final response NOW based on those results.
```

Gemini now sees an explicit list of completed work on every subsequent agent turn, eliminating the "I should re-read the instructions" reasoning.

**2. Strict one-shot rules added to system prompt:**
```
...
**3. Recursion limit lowered: `25 → 8`**

A single skill workflow needs at most 4 agent turns (initial → read_instructions → call_tool → respond). Limit of 8 gives a generous buffer while capping runaway loops:
```python
# skill_agent.py
...
**Flow after the fix:**
```
Turn 1: agent → calls read_skill_instructions + extract_youtube_transcript
Turn 2: tools execute, results returned
Turn 3: agent sees "✅ Tools Already Executed" block → writes final response
Turn 4: END
```
...
**Solution:** Already fixed with `st.rerun()` + `_pending_rerun` flag. If it regresses, ensure the pending rerun check is at the very top of `app.py` before any `st.` rendering call.
...
### Transcript response is blank (skills + token badge show, no text)

**Cause:** See **Bug 1** in Section 15. Either the list-comprehension re-processing in `app.py` was stripping the response, or Gemini returned an empty synthesis.  
**Solution:** Fixed — `app.py` now uses `result["response"]` directly and has a fallback renderer that shows raw tool output if the LLM synthesis is empty.

### `'FetchedTranscriptSnippet' object is not subscriptable`

**Cause:** See **Bug 2** in Section 15. `youtube-transcript-api` v1.x returns objects, not dicts.  
**Solution:** Fixed — `extract_transcript.py` normalises segments via `_segment_to_dict()`.

### Agent calls the same tools twice (double-loop)

**Cause:** See **Bug 3** in Section 15. Gemini re-entered the workflow loop after getting results.  
**Solution:** Fixed — dynamic "already done" block in the system prompt + recursion limit lowered to 8.
...
**Solution:** Check that `SERPAPI_API_KEY` is set in the sidebar. The script returns detailed error information in the `errors` field of the result.
...
**Solution:** `extract_text_content()` in `skill_agent.py` handles this. Ensure it is called on `last_msg.content` (not the raw message object).
...
*Document generated: 2026 | Project: LangChain Skills Agent | Stack: Gemini + LangGraph + Streamlit | Last updated: Bug fixes — transcript display, youtube-transcript-api v1.x compat, agent double-loop*
